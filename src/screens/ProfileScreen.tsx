...
export function ProfileScreen() {
  ...
  const onSubmitDisplayName = () => {
    ...
    if (!nextName) {
      Alert.alert('Display name required', 'Enter a name before saving.');
      return;
    }

    if (nextName.length < 2 || nextName.length > 30) {
      Alert.alert('Use 2 to 30 characters', 'Display names must be between 2 and 30 characters.');
      return;
    }
    ...
    Alert.alert('Change display name?', 'You can only change your display name once every 30 days.', [
      {
        text: 'Cancel',
        style: 'cancel',
      },
      {
        ...
        onPress: () => {
          updateDisplayName.mutate(
            ...
            {
              onSuccess: () => {
                Alert.alert('Display name updated', 'Your new display name is now active.');
              },
              onError: (error) => {
                Alert.alert('Could not update display name', error.message || 'Please try again.');
              },
            },
          );
        },
